...
var svg = d3.select("#map").append("svg").attr("preserveAspectRatio", "xMinYMin meet").attr("viewBox", "0 0 960 500").append("g");
...
Promise.all([d3.json("https://cdn.jsdelivr.net/npm/world-atlas@2.0.2/countries-110m.json"), d3.json("/assets/json/countries.json")]).then(function ([
  world,
  countries,
]) {
  ...
  svg
    ...
    .on("mouseover", function (d) {
      ...
      if (countryNames.includes(d.properties.name)) {
        ...
        tooltip
          .html(tip ? countryName + ", " + tip : countryName)
...
svg.append("rect").attr("width", width).attr("height", height).attr("fill", "transparent").lower(); // Send it to the back
